...
$(function () {
  ...
  $('.activity, .activity-body .activity-close').on('click', function () {
      $('.activity-body').toggleClass('active');
      return false;
  });

  $(document).mouseup(function (e) {
      var container = $(".activity-body .floated-chat-i");
      if (!container.is(e.target) && container.has(e.target).length === 0) {
          $(".activity-body").removeClass('active');
      }
  });

  $('.add-on, .add-on-body .close').on('click', function () {
      $('.add-on-body').toggleClass('active');
      return false;
  });
