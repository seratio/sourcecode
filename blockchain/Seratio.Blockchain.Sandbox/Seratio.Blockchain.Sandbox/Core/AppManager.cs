...
namespace Seratio.Blockchain.Sandbox.Core
{
    public class AppManager
    {
        ...
        public static bool IsETHSyncing;
        ...
        public static async Task<AppUser> AuthenticateUserAsync(int ID)
        {
            try
            {
                ...
                if (_user != null)
                {
                    bool flag = !(_user.WalletAddress != null);
                    ...
                    return new AppUser
                    {
                        ...
                        CanAccessRewardingBody = db.ModuleAccessRequests.Any(i => i.UserID == _user.ID && i.Module == "Rewarding Body" && i.IsApproved == true),
                        CanAccessRetailEnd = db.ModuleAccessRequests.Any(i => i.UserID == _user.ID && i.Module == "Retail End" && i.IsApproved == true),
                        CanAccessProvinance = db.ModuleAccessRequests.Any(i => i.UserID == _user.ID && i.Module == "Provenance" && i.IsApproved == true),
        ...
        public static async Task<AppUser> AuthenticateAutologinUserAsync(int ID, Guid token, long ticks)
        {
            try
            {
                DateTime _createdOn = new DateTime(ticks);
                var _user = await (from u in db.Users
                                   where u.AutoLoginToken == token && u.ID == ID && u.CreatedOn == _createdOn && u.IsDataActive && u.ShouldAllowSignInWithQR &&
        ...
        public static async Task<string> SavePaperWallet(string url, string path)
        {
            if (File.Exists(path))
            {
                File.Delete(path);
        ...
        {
            DateTime _start = new DateTime(1970, 1, 1, 0, 0, 0, 0, System.DateTimeKind.Utc);
            DateTime _correctDateTime = _start.AddSeconds(unixTime);
