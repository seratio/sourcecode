    ...
    public class Scheduler
    {
        public static void Schedule()
        {
            try
            {
                ...
                //IJobDetail _externalTxConfirmationJob = JobBuilder.Create<ETHExplorerTxSyncer>().Build();
                //ITrigger _externalTxConfirmationJobTrigger = TriggerBuilder.Create()
                //    .WithDailyTimeIntervalSchedule(s => s.WithIntervalInSeconds(10).OnEveryDay()).Build();
                //_scheduler.ScheduleJob(_externalTxConfirmationJob, _externalTxConfirmationJobTrigger);
