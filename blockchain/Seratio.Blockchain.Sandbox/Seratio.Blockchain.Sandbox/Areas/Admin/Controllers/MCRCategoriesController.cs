...
namespace Seratio.Blockchain.Sandbox.Areas.Admin.Controllers
{
    ...
    public class MCRCategoriesController : BaseController
    {
        ...
        public async Task<JsonResult> List(DataTableRequest request, string daterange)
        {
            ...
            {
                _query = _query.Where(c => c.LastUpdatedOn.ToString().Contains(request.sSearch) || c.Symbol.Contains(request.sSearch) || c.Name.Contains(request.sSearch) || c.Address.Contains(request.sSearch) || c.Email.Contains(request.sSearch));
            }
            ...
            foreach (var _rowData in _data)
            {
                ...
                _row.Add("<a href='/wallet/admin/mcrcategories/details/" + _rowData.ID + "' >" + _rowData.Name + "</a>");
                ...
                {
                    _row.Add(@"<a href='/wallet/admin/mcrcategories/edit/" + _rowData.ID + "' title='Edit' class='mr5'><i class='os-icon os-icon-edit-1 text-primary'></i></a>");
                }
                else
                {
                    _row.Add(@"<a href='/wallet/admin/mcrcategories/edit/" + _rowData.ID + "' title='Edit' class='mr5'><i class='os-icon os-icon-edit-1 text-primary'></i></a>  <a class='confirm' data-confirm='Are you sure you want to cancel this Coin?' href='/wallet/admin/mcrcategories/delete/" + _rowData.ID + "' title='Delete'><i class='fa fa-trash'></i></a> ");
                }
        ...
        public async Task<ActionResult> Details(int ID)
        {
            ...
            Coin _coin = await db.Coins.FirstOrDefaultAsync(c => c.ID == ID && c.IsMCRCategory);
        ...
        public async Task<ActionResult> Create(Coin model, HttpPostedFileBase image)
        {
            if (ModelState.IsValid)
            {
                ...
                model.IsMCRCategory = true;
                model.NumberOfDecimals = 18;
                model.BlockchainID = await db.Blockchains.Where(b => b.IsDataActive && !b.IsPrimaryNode)
                    .Select(b => b.ID).FirstOrDefaultAsync();

                db.Coins.Add(model);
                await db.SaveChangesAsync();

                TempData["Notification"] = new Notification("Success", "MCR Category has been created successfully.");
        ...
        public async Task<ActionResult> Edit(Coin model, HttpPostedFileBase image)
        {
            if (ModelState.IsValid)
            {
                ...
                if (_coin != null)
                {
                    ...
                    _coin.LastUpdatedOn = AppManager.Now;

                    await db.SaveChangesAsync();

                    TempData["Notification"] = new Notification("Success", "Requested MCR Category has been saved successfully.");
                    return RedirectToAction("Index");
                }
                else
                {
            ...
            if (_coin != null)
            {
                if (_coin.Name.ToLower() != "ether" || _coin.Address != null)
                {
                    ...
                    TempData["Notification"] =
                        new Notification("Success", "Requested MCR Category has been deleted successfully.");
                    await db.SaveChangesAsync();
                }
                else
                {
