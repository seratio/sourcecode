...
contract ClassicDelta {
    ...
    function ClassicDelta(address admin_, address feeAccount_, address accountLevelsAddr_, uint feeMake_, uint feeTake_, uint feeRebate_) {
        admin = admin_;
    ...
    function trade(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce, address user, uint8 v, bytes32 r, bytes32 s, uint amount) {
        ...
        orderFills[user][hash] + amount <= amountGet
        )) throw;
    ...
    function tradeBalances(address tokenGet, uint amountGet, address tokenGive, uint amountGive, address user, uint amount) private {
        ...
        if (accountLevelsAddr != 0x0) {
            ...
            if (accountLevel==1) feeRebateXfer = amount * feeRebate / (1 ether);
